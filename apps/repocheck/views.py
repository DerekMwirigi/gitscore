...
from apps.repocheck.serializers import CheckGitHubOrganisationRepositoryReponse_Serializer, CheckGitHubRepositoryReponse_Serializer
...
organisation_repositories_response = openapi.Response('response description', CheckGitHubOrganisationRepositoryReponse_Serializer)
@swagger_auto_schema(method='get', manual_parameters=[organisation], responses={200: organisation_repositories_response})
@api_view(['GET'])
@permission_classes([IsAuthenticated])
@authentication_classes([TokenAuthentication])
def check_github_organisation_repositories_score(request, organisation):
    reponse = { 'status': False, 'message': 'Failed', 'errors': [], 'data': []}
    status_code = status.HTTP_200_OK
    url = 'https://api.github.com/orgs/{}/repos'.format(organisation)
    http_reponse = requests.get(url=url)
    status_code = http_reponse.status_code
    repositories = []
    if status_code == 200:
        repo_json_response = http_reponse.json()
        for repository in repo_json_response:
            repositories.append(evaluate_repository(repository))
        reponse = { 'status': True, 'message': 'Success', 'errors': [], 'data': repositories}
